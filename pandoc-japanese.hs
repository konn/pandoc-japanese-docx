...
import           Text.Highlighting.Kate            (Style, espresso, haddock,
                                                    kate, languages, monochrome,
                                                    pygments, tango, zenburn)
import           Text.Pandoc
...
import           Text.Pandoc.Readers.Docx.Japanese (readDocxJP)
import           Text.Pandoc.Readers.LaTeX         (handleIncludes)
import           Text.Pandoc.Readers.Txt2Tags      (getT2TMeta)
import           Text.Pandoc.SelfContained         (makeSelfContained)
import           Text.Pandoc.Shared                (err, headerShift, normalize,
                                                    openURL, readDataFile,
...
main = do
  ...
  reader <- if "t2t" == readerName'
              ...
              else if "docx-japanese" == readerName'
              then return $ ByteStringReader $ (return .) . readDocxJP
