...
module Text.Pandoc.Readers.Docx.Japanese
       ( readDocxJP
       ) where
...
readDocxJP :: ReaderOptions
           -> B.ByteString
           -> (Pandoc, MediaBag)
readDocxJP opts bytes =
  ...
  | Just (_, cs) <- rStyle rPr =
      ...
      in
       RunStyle { isBold = case isBold rPr of
                ...
                , isTatechuYoko = case isTatechuYoko rPr of
                     Just bool -> Just bool
                     Nothing   -> isTatechuYoko rPr'
                ...
                , rStyle = rStyle rPr
                , rEmphasis = case rEmphasis rPr of
                     Just em -> Just em
                     Nothing -> rEmphasis rPr'
                    }
  | otherwise = rPr
...
runStyleToTransform rPr
  ...
  | Just DotE <- rEmphasis rPr =
      spanWith ("", ["bullet"], []) . (runStyleToTransform rPr {rEmphasis = Nothing})
  | Just CommaE <- rEmphasis rPr =
      spanWith ("", ["sesame"], []) . (runStyleToTransform rPr {rEmphasis = Nothing})
  | Just True <- isTatechuYoko rPr =
      spanWith ("", ["tcy"], []) . (runStyleToTransform rPr {isTatechuYoko = Nothing})
  | otherwise = id
...
runToInlines (Ruby rbs rt base) = do
    ils <- runToInlines rt
    rls <- runToInlines base
    return $ buildRuby rbs ils rls
...
buildRuby :: RubyStyle -> Inlines -> Inlines -> Inlines
buildRuby rbs rt base =
  let ats = [("align", show $ rubyAlign rbs)
            ,("rubySize", show $ rubySize rbs)
            ,("baseSize", show $ rubyBaseSize rbs)
            ,("rubyRaise", show $ rubyRaise rbs)
            ]
  in spanWith ("", ["ruby"], ats) $
     Many $ unMany rt <> unMany base
