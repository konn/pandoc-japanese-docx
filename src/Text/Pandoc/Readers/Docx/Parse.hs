{-# LANGUAGE FlexibleInstances, PatternGuards, ViewPatterns #-}
...
import           Text.Pandoc.Readers.Docx.Except
import           Text.Pandoc.Readers.Docx.Fonts  (Font (..), getUnicode)
...
data RubyStyle = RubyStyle { rubyAlign    :: Maybe RubyAlign
                           , rubySize     :: Maybe Integer
                           , rubyRaise    :: Maybe Integer
                           , rubyBaseSize :: Maybe Integer
                           , languageId   :: Maybe String
                           }
                 deriving (Show)

defaultRubyStyle :: RubyStyle
defaultRubyStyle = RubyStyle { rubyAlign = Nothing
                             , rubySize  = Nothing
                             , rubyRaise = Nothing
                             , rubyBaseSize = Nothing
                             , languageId = Just "ja-JP"
                             }

data Emphasis = DotE | CommaE
              deriving (Show)
...
data RunStyle = RunStyle { isBold        :: Maybe Bool
                         ...
                         , isStrike      :: Maybe Bool
                         , isTatechuYoko :: Maybe Bool
                         , rVertAlign    :: Maybe VertAlign
                         , rUnderline    :: Maybe String
                         , rStyle        :: Maybe CharStyle
                         , rEmphasis     :: Maybe Emphasis
                         }
                deriving Show
...
elemToRun ns element
  | isElem ns "w" "r" element
  , Just ruby <- findChild (elemName ns "w" "ruby") element = do
    (rt, base) <- elemToRubyElems ns ruby
    rbStyle <- elemToRubyStyleD ns ruby
    return $ Ruby rbStyle rt base
elemToRun ns element
...
elemToRubyElems :: NameSpaces -> Element -> D (Run, Run)
elemToRubyElems ns rb
  | Just rt <- findChild (elemName ns "w" "r")
               =<< findChild (elemName ns "w" "rt") rb
  , Just base <- findChild (elemName ns "w" "r")
               =<< findChild (elemName ns "w" "rubyBase") rb = do
    liftM2 (,) (elemToRun ns rt) (elemToRun ns base)
elemToRubyElems _ _ = throwError WrongElem
...
elemToRubyStyleD :: NameSpaces -> Element -> D RubyStyle
elemToRubyStyleD ns element
  | Just rPr <- findChild (elemName ns "w" "rubyPr") element = do
    return $ elemToRubyStyle ns rPr
elemToRubyStyleD _ _ = return defaultRubyStyle

elemToRubyStyle :: NameSpaces -> Element -> RubyStyle
elemToRubyStyle ns element
  | Just rbPr <- findChild (elemName ns "w" "rubyPr") element =
    RubyStyle { rubyAlign =
                  findChild (elemName ns "w" "rubyAlign") rbPr
                  >>= findAttr (elemName ns "w" "val")
                  >>= \v -> Just $ case v of
                    "center" -> Center
                    ...
                    _ -> DistributeLetter
              , rubySize =
                    findChild (elemName ns "w" "hps") rbPr
                    >>= findAttr (elemName ns "w" "val")
                    >>= stringToInteger
              , rubyBaseSize =
                    findChild (elemName ns "w" "hpsBaseText") rbPr
                    >>= findAttr (elemName ns "w" "val")
                    >>= stringToInteger
              , rubyRaise =
                    findChild (elemName ns "w" "hpsRaise") rbPr
                    >>= findAttr (elemName ns "w" "val")
                    >>= stringToInteger
              , languageId =
                    findChild (elemName ns "w" "lid") rbPr
                    >>= findAttr (elemName ns "w" "val")
              }
elemToRubyStyle _ _ = defaultRubyStyle
...
elemToRunStyle ns element parentStyle
  | Just rPr <- findChild (elemName ns "w" "rPr") element =
    RunStyle
      ...
      , isTatechuYoko = fmap valToBool
                        (findChild (elemName ns "w" "eastAsianLayout") rPr
                        >>= findAttr (elemName ns "w" "vertCompress"))
      ...
      , rStyle = parentStyle
      , rEmphasis =
           findChild (elemName ns "w" "em") rPr >>=
           findAttr (elemName ns "w" "val") >>=
           \v -> Just $ case v of
             "dot"   -> DotE
             "comma" -> CommaE
